...
class SafePath:
    ...
    def set_destination(self,road_df,start,end):
        ...
        # 밝기 정보 정규화하기
        road_df['밝기'] = minmax_scale(road_df['밝기'], axis=0, copy=True, feature_range=(0, 1000))
    ...
    def find_path(self,df, radius=50):
        ...
        def get_f(df):
            ...
            # 2. Scaled_F : 거리 0~100 에 맞추기
        def get_scaled_f(df):
            X_MinMax_scaled = 1000 - minmax_scale(df['F'], axis=0, copy=True, feature_range=(0, 1000))
            return X_MinMax_scaled

            # 3. 밝기 이용한 가중치 추가 W(weight = 4F(거리) + 6L(밝기)) => 조절해보기
        def get_w(df, w_lux=35, w_f=65):
        ...
        while (sel_pointID != 'POINT_EP'):
            ...
            del_pointID = points.loc[1:, 'POINT_ID']
            self.searched_points = points.loc[1:, 'POINT_ID']
            ...
            df = df[~df.POINT_ID.isin(del_pointID)]
            print(len(df))
            n += 1
